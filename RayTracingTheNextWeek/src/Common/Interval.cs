namespace RayTracingTheNextWeek.Common
{
    internal struct Interval<T> where T : IComparable<T>
    {
        ...
        public Interval(Interval<T> a, Interval<T> b)
        {
            Min = a.Min.CompareTo(b.Min) < 0 ? a.Min : b.Min;
            Max = a.Max.CompareTo(b.Max) > 0 ? a.Max : b.Max;
        }
        ...
        public static Interval<T> operator +(in Interval<T> a, T b)
        {
            return new Interval<T>((dynamic)a.Min + b, (dynamic)a.Max + b);
        }

        public static Interval<T> operator +(T a, in Interval<T> b)
        {
            return new Interval<T>(a + (dynamic)b.Min, a + (dynamic)b.Max);
        }
